...
### Data Loading
The ELO table named *tournamentinfo.txt* is loaded with the \textbf{read.delim()} function from my publicly accessible GitHub repo. As expected, the resulting *eloTable* is an R data frame but not an easy format for analysis.
```{r}
eloTable <-
...
Finally, we need summation of opponent's pre ratings which are placed in multiple columns. This is also not the friendly format for summation whose operation, like any other statistical ones, usually works in a column/variable. The \textbf{rowSums()} function will do.
```{r}
...
Like any other principles, data analysis has well-established rules, some of which presented in this project. It is a good practice to keep that in mind, follow the rules, and extend the rules if necessary. This would be a guideline to us when we face unusual formats of data.
